...
router.get("/user", isLoggedIn, roleBaseAuthoriza("User", "Manager", "Admin"), (req, res) => {
    res.status(200).json({ message: "User Page & Access all" });
});
router.get("/admin", isLoggedIn, roleBaseAuthoriza("Admin"), (req, res) => {
    res.status(200).json({ message: "Admin Page & Access only [Admin]" });
});
router.get("/manager", isLoggedIn, roleBaseAuthoriza("Manager", "Admin"), (req, res) => {
    res
        .status(200)
